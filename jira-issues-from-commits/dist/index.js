    ...
    function patch(fs3) {
      ...
      function patchLchmod(fs4) {
        fs4.lchmod = function(path, mode, callback) {
          fs4.open(path, constants.O_WRONLY | constants.O_SYMLINK, mode, function(err, fd) {
            if (err) {
              if (callback)
                callback(err);
              return;
  ...
  "node_modules/fs-extra/lib/remove/rimraf.js"(exports, module2) {
    ...
    function rmkids(p, options, cb) {
      ...
      options.readdir(p, (er, files) => {
        ...
        files.forEach((f3) => {
          rimraf(path.join(p, f3), options, (er2) => {
            if (errState) {
              return;
            }
            if (er2)
    ...
    function rmkidsSync(p, options) {
      assert(p);
      assert(options);
      options.readdirSync(p).forEach((f3) => rimrafSync(path.join(p, f3), options));
      if (isWindows) {
...
var require_remove2 = __commonJS({
  "node_modules/fs-extra/lib/remove/index.js"(exports, module2) {
    ...
    function remove(path, callback) {
      if (fs3.rm)
        return fs3.rm(path, { recursive: true, force: true }, callback);
      rimraf(path, callback);
    }
    function removeSync(path) {
      if (fs3.rmSync)
        return fs3.rmSync(path, { recursive: true, force: true });
      rimraf.sync(path);
    }
    module2.exports = {
      remove: u(remove),
...
var issueTypeSort = (issueType) => getIssueTypeObject(issueType)?.sort ?? 999;
async function run() {
  try {
    ...
    const jiraIssueKeys = /* @__PURE__ */ new Set();
    ...
    await import_fs_extra.default.outputJSON(outputFile, output, { encoding: "utf-8" });
    core.info(`Complete!`);
  } catch (error) {
